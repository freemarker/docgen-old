...
[#macro breadcrumb]
  ...
  [#assign curNode = .node]
  [#list 1..99 as _]
    [#assign path = [curNode] + path]
    [#if curNode.@docgen_root_element?size != 0]
      [#break]
    [/#if]
    [#assign curNode = curNode?parent]
  [/#list]
  [#if path?size > 1]
  ...
  [#if internalBookmarks?size != 0 || externalBookmarks?size != 0]
    <div class="bookmarks">
      <span class="bookmarks">[#t]
        Bookmarks:[#lt]
        [#assign curHref = CreateLinkFromNode(.node)]
        [#list internalBookmarks?keys as k]
          [#assign target = CreateLinkFromID(internalBookmarks[k])]
          [#if target != curHref]
...
[#macro pagerButton label element labelOnly=true]
  <div class="pagerButton">[#t]
    ...
    [#assign curHref = ""]
    [#if element??]
      [#assign href = CreateLinkFromNode(element)]
      [#assign curHref = CreateLinkFromNode(.node)]
    [/#if]
